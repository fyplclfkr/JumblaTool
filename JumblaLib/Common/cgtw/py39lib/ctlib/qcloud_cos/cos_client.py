...
class CosS3Client(object):
    ...
    def _upload_part(self, bucket, key, local_path, offset, size, part_num, uploadid, md5_lst, resumable_flag,
                     already_exist_parts, file_size, enable_md5, a_callback=None):
        ...
        if file_size != os.path.getsize(local_path):
            raise Exception(u"The file is modified, please upload again")
        if resumable_flag and part_num in already_exist_parts:
            ...
            # ----------add----------------
            self.m_has_upload_size = self.m_has_upload_size + size
            if a_callback != None and (type(a_callback) == types.FunctionType or type(a_callback) == types.MethodType):
                a_callback(self.m_has_upload_size, size, self.m_filesize)
            # ----------add----------------
    ...
    def upload_file(self, Bucket, Key, LocalFilePath, PartSize=1, MAXThread=5, EnableMD5=False, CallBack=None,
                    **kwargs):
        ...
        if file_size <= 1024 * 1024 * 20:
            with open(LocalFilePath, 'rb') as fp:
                ...
                # ----------add----------------
                if CallBack is not None and isinstance(CallBack, (types.FunctionType, types.MethodType)):
            ...
            self.m_has_upload_size = 0  # 已经上传的大小
            ...
            if len(exist_keys) > 0:
                ...
                if parts_num in exist_keys:
                    self.m_has_upload_size = last_size + (len(exist_keys) - 1) * part_size
                else:
                    # 没有的话;已上传大小=其他段的个数*part_size
                    self.m_has_upload_size = len(exist_keys) * part_size
            # ----------add----------------

            # 上传分块
            offset = 0  # 记录文件偏移量
            lst = list()  # 记录分块信息
            pool = SimpleThreadPool(MAXThread, flag_dict=self.flag_dict)

            for i in range(1, parts_num + 1):
                if i == parts_num:  # 最后一块
                    pool.add_task(self._upload_part, Bucket, Key, LocalFilePath, offset, file_size - offset, i,
                                  uploadid, lst, resumable_flag, already_exist_parts, file_size, EnableMD5, CallBack)
                else:
                    pool.add_task(self._upload_part, Bucket, Key, LocalFilePath, offset, part_size, i, uploadid, lst,
                                  resumable_flag, already_exist_parts, file_size, EnableMD5, CallBack)
                    offset += part_size
    ...
    def stop_upload(self):
        if hasattr(self, "flag_dict"):
            if isinstance(self.flag_dict, dict):
                self.flag_dict["is_pause"] = True
            else:
                self.flag_dict = {"is_pause": True}
