...
def to_complete_multipart_upload_request(parts):
    dic = {}
    p = []
    for part in parts:
...
def to_delete_multi_objects_request(objects, quiet):
    data = {}
    if objects:
        obs = []
        for o in objects:
...
def to_put_acl_request(owner, grants):
    data = {}
    if owner:
...
def to_fetch_object(url, object=None, ignore_same_key=None, content_md5=None):
    info = {'URL': url}
    if object:
        info['Object'] = object
    if ignore_same_key:
...
def to_put_replication(role, rules):
    info = {}
    if role:
        info['Role'] = role
    r = []
...
def to_put_bucket_website(redirect_all_requests_to,
                          index_document, error_document, routing_rules):
    info = {}
...
def to_put_custom_domain(custom_domain_rule):
    info = {}
...
def to_put_bucket_real_time_log(configuation):
    info = {}
    if configuation:
...
def to_restore_object(days, tier):
    info = {}
    if days:
        info['Days'] = days
