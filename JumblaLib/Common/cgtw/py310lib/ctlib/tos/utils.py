...
def parse_modify_time_to_utc_datetime(value):
    return datetime.datetime.strptime(value, LAST_MODIFY_TIME_DATE_FORMAT).replace(tzinfo=utc)


def parse_gmt_time_to_utc_datetime(value):
    return datetime.datetime.strptime(value, GMT_DATE_FORMAT).replace(tzinfo=utc)
...
def init_checkpoint_dir(checkpoint_file):
...
def generate_http_proxies(ip, port, user_name=None, password=None):
    # 未配置ip地址 返回空{}
...
def meta_header_encode(query, doseq=False, safe='', encoding=None, errors=None,
    ...
    if hasattr(query, "items"):
        ...
        try:
            ...
            tb = sys.exc_info()[2]
            six.reraise(TypeError, TypeError("not a valid non-string sequence "
...
class _IterableAdapter(object):
    def __init__(self, data, progress_callback=None, limiter_callback=None, download_operator=False,
                 can_reset=False):
        self.iter = iter(data)
        self.progress_callback = progress_callback
        self.limiter_callback = limiter_callback
...
class TokenBucketResult(object):
    def __init__(self, ok, time_to_wait):
        self.ok = ok
...
class DnsCacheService(object):
    ...
    def get_ip_list(self, host, port):
        now = int(time.time())
...
class MergeProcess(object):
    ...
    def __call__(self, consumed_bytes, total_bytes, rw_once_bytes, type):
        with self.lock:
