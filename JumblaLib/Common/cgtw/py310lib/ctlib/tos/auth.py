...
from .utils import to_bytes, _param_to_quoted_query, utc
...
def _canonical_query_string_params(params):
    ...
    for param in sorted(params):
        ...
        results.append('%s=%s' % (url_quote(param, safe='-_.~'),
                                  url_quote(value, safe='-_.~')))
    cqs = '&'.join(results)
    return cqs
...
def _canonical_request(req):
    cr = [req.method.upper(), url_quote(req.path, safe='/~'), _canonical_query_string_params(req.params),
...
def _get_post_policy(date, expire, algorithm, credential, bucket, object_key, conditions,
                     content_length_range=None, sts=None):
    time = datetime.datetime.strptime(date, DATE_FORMAT).replace(tzinfo=utc) + datetime.timedelta(seconds=expire)
    post_policy = {
...
class AuthBase(object):
    ...
    def post_sign(self, bucket, key, expires, conditions, content_length_range):
        date = datetime.datetime.utcnow().strftime(DATE_FORMAT)
        self.credential = self.credentials_provider.get_credentials()
    ...
    def x_tos_post_sign(self, expires, conditions):
        if expires is None:
            expires = 60 * 60
        date = datetime.datetime.utcnow().strftime(DATE_FORMAT)
        self.credential = self.credentials_provider.get_credentials()
