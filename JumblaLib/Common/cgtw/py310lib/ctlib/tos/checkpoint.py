...
from .utils import (SizeAdapter, to_unicode, MergeProcess, to_bytes,
                    rename_file, copy_and_verify_length, makedir_p)
if six.PY2:
    from concurrent2 import futures
else:
...
class CheckPointStore(object):
    ...
    def __init__(self, dir, file_name, use_type):
    ...
    def path(self, bucket, key, src_bucket=None, src_key=None, versionId=None):
        ...
        if src_bucket and src_key:
            ...
            if versionId:
                encode_str += '.' + versionId
        encoding_data = url_quote(hashlib.md5(to_bytes(self.file_name + encode_str)).digest(), safe='')

        name = "{0}.{1}".format(encoding_data, self.suffix)
        return os.path.join(self.dir, name)


def _cover_to_uploaded_parts(parts):
    """
...
class BreakpointBase(object):
    def __init__(self, client, bucket, key, store, task_num, parts_to_do, record, size, rate_limiter, cancel_hook,
...
class _BreakpointUploader(BreakpointBase):
    def __init__(self, client, bucket, key, store, task_num, parts_to_update, upload_id, record,
                 size, ssec_algorithm, ssec_key, ssec_key_md5, rate_limiter, cancel_hook,
                 datatransfer_listener, upload_event_listener, file_path, traffic_limit):
...
class _BreakpointDownloader(BreakpointBase):
    def __init__(self, client, bucket, key, file_path, store, task_num, parts_to_download, record, etag,
...
class TaskExecutor(object):
    def __init__(self, num_task, task_fun, cancel_hook):
        self.executor = futures.ThreadPoolExecutor(max_workers=num_task)
